<?php
    ...
    function SetPushToken($PushToken, $UName, $conn, $db) {
        $table = $db.".push_notifications_tokens_roadrunners";
    ...
    function LogInRegisterUser($db, $tb, $c, $u, $p, $push) {
        ...
        $SQL_UPDATE_TOKEN = "UPDATE $theTable SET `Session_Token` = ? WHERE `Email` = ? AND `PasswordMD5` = ?";
        $SQL_COMPARE_PASS = "SELECT * FROM $theTable WHERE `Email` = ? AND `PasswordMD5` = ?";
