<?php
    ...
    if (file_get_contents("php://input") != null) {
        ...
        function confirmApiKey($key, $sr, $db, $un, $ps) {
            ...
            $SQL_CONFIRM_KEY = "SELECT * FROM $table WHERE `Session_Token` = ?";
        ...
        function GetUserInfo($key, $sr, $db, $un, $ps) {
            ...
            $conn = new PDO("mysql:host = $sr; dbname = $db", $un, $ps, $arrOptions);
            $conn -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $SQL_GET_USER_INFO = "SELECT `UserName`, `LastName`, `Picture_Name`, \n
            `PhoneNumber`, `Monthly_Money_Gained`, `Requests_Accomplished`, `City`, `Country`, \n
            `Deliver_Rating` FROM $table WHERE `Session_Token` = ?";
